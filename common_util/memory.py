...
def get_entities(use_long_cache:bool=False, entity_tokens:int=2000):
    global ENTITY_MEMORY, entity_memory_long_cache
...
def get_filtered_entities(entities, entity_tokens, query):
    ...
    if tiktoken_len(json.dumps(entities))>3000:
        ...
        filtered_ent1 = get_filtered_entities(ent1,entity_tokens,query)
        filtered_ent2 = get_filtered_entities(ent2,entity_tokens,query)
        entities = {**filtered_ent1, **filtered_ent2}
        return get_filtered_entities(entities,entity_tokens,query)
    #TODO will break if entities is more than 4k tokens. need to split if this happens
        ...
        You can select tangentially related entities.
...
print(get_filtered_entities(TEST, 2000, "How does chain of thought prompting work?"))
#%%